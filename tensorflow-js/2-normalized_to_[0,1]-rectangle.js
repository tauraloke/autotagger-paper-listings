...
async function getSortedTags(filepath) {
  ...
  return tf.tidy(() => {
    let tensor = tf.node.decodeImage(fs.readFileSync(filepath), 3);
    console.log(tensor.shape, filepath);
    let ratio =
      (1.0 * PREPARED_IMAGE_SIZE) / Math.max(tensor.shape[0], tensor.shape[1]);
    let sizes = [224, 224
      //Math.floor(tensor.shape[0] * ratio),
      //Math.floor(tensor.shape[1] * ratio),
    ];
    console.log(sizes, filepath);
    tensor = tensor.resizeBilinear(sizes);
